...
class MenuBlockRepository implements MenuBlockRepositoryInterface {
   ...
   *   The request object.
   */
  public function __construct(Connection $database, EntityManagerInterface $entity_manager, CacheBackendInterface $cache, ModuleHandlerInterface $module_handler, ConfigFactory $config_factory, Request $request) {
  ...
  public function blockData(&$config) {
    ...
    // Localize the tree.
    if ($this->moduleHandler->moduleExists('i18n_menu')) {
    ...
    if ($config['level'] > 1 || $config['parent_mlid']) {
      if ($config['parent_mlid']) {
        ...
        // @todo circular dependency alert.
